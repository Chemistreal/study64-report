...
앞면은 시작 전에 읽는 넷이고 뒷면은 하면서 쓰는 둘이다.
한 면에 다 넣으려고 재 봤더니 본문이 59줄에서 84줄이었다. A4 한 면은 57줄이다.
T77 에서 쟀고 그때 앞뒤로 갈랐다. 갈라 놓으니 앞면 최대 46줄 뒷면 최대 44줄이다.
...
def render(num, title, one, eng, engsrc, split, segs, cards, sec, med, rec, notes, stuck, role, track, quarter):
    ...
    if med:
        ...
        L.append("미디어 없음")
    L.append("")
    # 여기가 종이의 접히는 자리다. 앞면 넷은 시작 전에 읽고 뒷면 둘은 하면서 쓴다.
    L.append("---")
    L.append("")
    L.append("**여기서 뒤로 넘긴다.** 앞면은 시작 전에 읽고 뒷면은 하면서 쓴다.")
    L.append("")
    L.append("## 5. 기록 칸")
    L.append("")
