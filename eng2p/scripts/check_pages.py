...
JOBS = [("audit_pages.py", "대비와 글씨와 뼈대"),
        ...
        # 2026-08-10 에 하나 더 (PR #17). **이 자는 자를 잰다.**
        # CI 가 부르는 자가 거기서 정말 돌 수 있는지를 본다. 08:44 부터 여섯 판이
        # 다 빨간불이었는데 나는 손에서만 돌려 보고 초록이라고 말하고 있었다.
        # **돌 수 있는 것과 걸어 둔 것은 다르다.**
        ("ci_deps.py", "CI 가 부르는 자가 거기서 돌 수 있나"),
        # 2026-08-10 에 하나 더 (PR #18). **화면과 글이 아니라 건네는 파일 안을 본다.**
        # 수업 PDF 쉰둘 중 넷에 선생님용 답이 실려 있다. 그 넷은 알고 있고
        # 다섯째가 생기면 빨간불이다. 어떻게 할지는 사용자가 정할 칸이다.
        ("worksheet_leak.py", "건네는 학습지에 답이 새로 실렸나")]
