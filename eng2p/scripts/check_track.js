...
(async () => {
  ...
  /* **0 / 0 을 안 적는다.** Q1 에 문법과 자동화 배정이 0이다 */
  ["문법", "자동화"].forEach((t) => {
    if (!by[t]) { no(t + " 줄이 없다"); return; }
    if (by[t][2].indexOf("이 분기에는 없다") < 0)
      no(t + " 의 이 분기 칸이 '" + by[t][2] + "' 다. 없다고 적어야 한다");
    if (/^0/.test(by[t][2]))
      no(t + " 의 이 분기 칸이 0으로 시작한다. 0을 보이면 못 한 것으로 읽힌다");
  });
  /* 배정이 있는 트랙은 숫자를 적는다 */
  if (!by["소리"] || by["소리"][2].indexOf("15") < 0)
    no("소리의 이 분기 칸이 '" + (by["소리"] || [])[2] + "' 다. 15강이어야 한다");
  ...
  console.log("트랙 진도 22판 (진도 6, 0 적기 5, 남은 것 5, 처음과 끝 6) / 실패 %d",
              fails.length);
