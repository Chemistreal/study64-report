...
# 사람별로 갈려도 되는 뿌리. **`docs/gap.md` 3장 표가 원본이다.**
#
# 넷은 다 글이었다. 다섯째(`cardDue`)가 T358 에 늘었고 **그것은 숫자다.**
# 숫자는 견줄 수 있으므로 막는 자리가 옮겨 간다.
# 값을 못 만들게 막는 것에서 **화면에 못 놓게 막는 것**으로다.
# 화면 쪽은 `check_versus.js` 가 잰다. 둘 다 있어야 그 자리가 지켜진다.

OK_ROOT = {"names", "aim", "xchk", "rel", "cardDue"}
...
NOT_PERSON = {
    ...
    "c": "`cardSplit` 이 돌려주는 갈래 모음을 담는 변수다. 그 모음 안이 `cardDue` 다",
    "A": "합치기가 이쪽 기록을 담는 변수다. 갈래 모음이지 사람이 아니다",
    "B": "합치기가 저쪽 기록을 담는 변수다. 같은 까닭이다",
    "out": "합친 결과를 담는 변수다. 갈래 모음이다",
}
