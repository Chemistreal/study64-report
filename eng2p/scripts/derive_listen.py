...
# 한 판이 여섯 줄이다 (규칙서 3.3 끝 조건).
#
# **아홉이던 것을 서른여섯으로 올렸다** (T416). 아홉은 "넉넉히" 로 정한 수였고
# 재 보니 넉넉하지 않았다. `check_supply.js` 가 이 판을 288일 돌려 보고
# **안 잼** 으로 적었다. 자루가 아홉인데 어제 여섯 오늘 여섯을 내면
...
# 그 엿새 동안 어제 들은 줄이 오늘 또 나오지 않으려면 자루가 그만큼은 돼야 한다.
# 대본에서 나오는 것이 과마다 평균 스물둘이라 **이 뚜껑은 대개 안 걸린다.**
#
# **고르는 규격은 안 건드렸다.** 낱말 넷과 지시 넷과 갈래를 고루 주는 셈은 그대로다.
# 늘린 것은 담는 수뿐이고 무엇을 지시로 삼는가는 그대로다.
PER = 36
MIN_WORDS = 4
