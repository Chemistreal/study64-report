...
docs/sound.md         소리 자리 열. **읽는 자리는 하나다.** 둘이 되면 검사가 잡는다
docs/hstage.md        소리 판정 H단계 결산. **절반은 안 재기로 한 것을 지키는 일이었다**
docs/cando.md         이 기기에서 안 될 수 있는 것 여덟. **문제만 적고 대응을 안 적으면 겁주기다**
docs/tone.md          말투. **다그치지 않는다.** 같은 사실을 두 가지로 적을 수 있으면 안 다그치는 쪽
                      10장이 판정 안 한다는 말이 뜨는 자리 다섯. **문장을 안 합친다**
...
docs/play_supply.md   자루가 도는 속도. **자료가 아니라 파생기의 뚜껑이 걸려 있었다**
docs/verify_plan.md   B등급 106건을 갈랐다. **71건은 검증이 아니라 확정이나 실행 대기다**
...
scripts/check_relation.js 관계 점검과 신호 75판. **따로 적고 같이 편다**
...
scripts/check_space.js 여백 단 552판. **개수만 세면 값이 바뀌는 것을 못 잡는다**
scripts/check_radius.js 모서리 단 139판. **알약은 사다리 밖이라 이름을 달리 준다**
...
**에이전트를 최대한 많이 만든다.** 나눌 수 있는 일은 갈라서 한꺼번에 돌린다.

이전에는 조수를 안 썼다. 조수가 만든 것을 검출할 사람이 없다는 것이 까닭이었다.
**그 걱정은 그대로다.** 바뀐 것은 검출을 사람이 아니라 검사기가 한다는 것이고,
검사기는 누가 만든 것인지 안 가린다. 그래서 늘릴 수 있게 됐다.

에이전트를 쓴다고 검사를 건너뛰지 않는다. 오히려 반대다.
**검사기가 유일한 외부 눈이다.** 에이전트가 만든 것도 `python3 scripts/all.py` 를
그대로 통과해야 하고 깸 시험도 그대로 한다.

### 가를 때 지키는 것

| 규칙 | 왜 |
|---|---|
| **파일 영역을 겹치지 않게 나눈다** | 같은 파일을 둘이 고치면 한쪽이 조용히 지워진다 |
| **파생물은 아무도 안 만든다** | `english.html` 과 `out/app/*.js` 는 마지막에 한 번만 뽑는다 |
| **공용 파일은 한 사람이 맡는다** | `all.py` `CLAUDE.md` `roadmap.md` `friction.md` `journal.md` |
| **에이전트는 커밋하지 않는다** | 한 턴이 한 커밋이다. 합치는 것은 맡은 쪽이 한다 |
| **검사기를 만들면 깸 시험까지 한다** | 안 잡히는 검사기는 통과한 것처럼 보인다 |
| **못 한 것은 못 했다고 보고한다** | 늘린 척한 숫자가 제일 나쁘다 |
