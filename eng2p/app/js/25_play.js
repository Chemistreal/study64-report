...
/* 자리를 바꾸고 싶을 때 (T348). **앱이 안 바꿔 준다.**
   ...
   앞은 앱이 한다. 뒤는 **앱이 못 한다.** 두 기기가 회 번호로 각각 셈해서
   자리를 낸다. 한쪽에서만 뒤집으면 둘 다 같은 자리가 되고 그 판이 조용히 깨진다.
   기기끼리 말할 길이 없어서 그것을 앱이 못 막는다 (round.md 2장).

   **그래서 바꾸는 길을 회로 낸다.** 회를 하나 넘기면 자리가 바뀐다.
   회는 두 기기가 각각 넘기고 짝 코드가 그것이 같은지를 본다.
   이미 있는 길이라 새로 만들 것이 없다.
