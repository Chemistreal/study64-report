...
function renderMediaList(){
  ...
  /* **같은 목록이면 다시 안 만든다.**
     52개 항목에 단추가 104개다. 넷 배 느린 기기에서 이 탭을 여는 데 224ms 였다.
     그런데 블록 1은 매 세션 이 탭을 연다. 목록이 그대로인데 매번 새로 만들었다.
     무엇이 그려질지를 한 줄로 적어 두고 그것이 같으면 넘어간다.
     블록 칸이 쓰는 `PANE.sig` 와 같은 수다. T186 */
  var sig=[q,state,needle,LIB.active,LIB.mode,shown.length,
           shown.map(function(o){ return o.x.id+":"+mediaPassCount(o.x.id)+(r.fav[o.x.id]?"f":""); }).join(",")].join("|");
  if(LIB.sig===sig && box.firstChild) return;
  LIB.sig=sig;
  box.innerHTML="";
