/* 자료 탭과 규칙 탭. **둘 다 거의 안 여는 자리다.** T361 뒤

   `20_docs.js` 에 있던 것을 여기로 뺐다. 열자마자 읽는 것이 천장까지 4KB 남았고
   갚을 자리를 **크기가 아니라 언제 쓰는가**로 골랐다 (T331 뒤에 정한 규칙).

   자료 탭은 조준표를 보는 자리고 규칙 탭은 표를 보는 자리다.
   T335 에 "규칙 탭은 아무도 안 연다" 고 적었다. 그 말이 곧 이 자리의 근거다.

   ## 규칙 탭이 시작할 때 그려지고 있었다

   `renderRules()` 를 시작에서 한 번 부르고 **탭을 열 때는 안 불렀다.**
   그러니 화면은 언제나 시작할 때의 값이었다. 여는 탭에서 그리게 고쳤다.
   늦게 읽는 자리로 오면서 그것이 저절로 맞았다. */
var SOURCES=[
