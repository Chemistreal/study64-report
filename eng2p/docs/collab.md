...
신뢰도: A 생성 (제작 관리)
지위: **종료.** 2026-08-07 부로 단독 개발로 전환했다. 이 문서는 기록이다.
우선순위: spec.md > audio_intake.md > roadmap.md > CLAUDE.md

---

## 종료 기록

조수 체제는 PR #2, #3, #4 와 T-009 까지 돌았다. 남긴 것과 배운 것은 이렇다.

**남긴 것.** VOA Level 1 52과. 음성, 대본, 이미지, 수업 PDF, 카탈로그.
이건 그대로 쓴다. 라이선스가 확인된 퍼블릭 도메인 자료다.

**배운 것 하나.** 합격 기준을 숫자로만 주면 최단 경로로 통과한다.
T-009 에서 "경고 0" 만 걸었더니 화자 수가 내려가고 분기가 한쪽으로 몰렸다.
규격 위반은 아니었다. 조건이 무엇을 위한 것인지는 지시서에 안 적혀 있었다.

**배운 것 둘.** 사람이 적는 숫자는 검증할 수 없으면 결국 조정된다.
화자 수를 대본에서 파생시키자 그 문제가 사라졌다.
앞으로 만드는 모든 값도 가능하면 파생값으로 둔다.

**배운 것 셋.** 조수와 나는 같은 실패 모드를 갖는다.
그래서 조수를 뺀다고 위험이 줄지 않는다. 검사기만 남는다.
...
| (단독 전환 후 전부 Claude 소유) | Claude | 2026-08-07 부로 소유권 구분이 없어졌다 |
...
`track` 은 2026-08-07 에 카탈로그에서 뺐다.
52개가 전부 소리로 일괄 입력돼 근거가 없었고,
트랙은 자료의 속성이 아니라 그 자료를 쓰는 강의의 속성이다.
강의 머리말이 자기 트랙을 선언한다.
