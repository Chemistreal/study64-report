...
### 1순위. T-009 화자 수 초과 17건 정리

지금 이 명령이 경고 17건을 낸다.

   python3 eng2p/scripts/check_media.py | grep 화자

기준서 10.2가 분기별 재료 조건을 정한다.
Q1 은 화자 1~2인, Q2 는 2인, Q3 은 2~3인이다.
lle1-32 는 9명, lle1-33 은 6명으로 잡혀 있다.

각 건에 대해 둘 중 하나를 한다.

1. 화자 수를 잘못 셌으면 고친다. 대본을 열어 실제로 센다
2. 제대로 셌으면 그 레슨의 quarter 를 조건에 맞는 분기로 옮긴다

옮기면 그 분기의 레슨 수가 흔들린다. 흔들린 결과를 PR 본문에 적는다.

합격 기준
- check_media.py 실패 0, 경고 0
- catalog 의 speakerCount 와 대본 파일의 "화자 수" 가 서로 같다
- check.py media/english/transcripts/ 도 경고 0

하지 말 것
- 경고를 없애려고 화자 수를 낮춰 적기. 그건 데이터를 망치는 것이다
- 검사기의 상한을 고치기. 상한은 기준서 10.2에서 온다

### 2순위. T-003 축약 20종 실제 용례 채집
...
3순위 이후는 eng2p/tasks/gpt_backlog.md 의 T-004 부터 T-008 을 본다.
순서는 그 파일의 표를 따른다.
```
