...
**종료. 2026-08-07 부로 단독 개발로 전환했다.**
이 파일은 기록이다. 새 작업은 여기서 나가지 않는다.
남은 T-003 부터 T-008 은 필요하면 `docs/roadmap.md` 안으로 흡수한다.
...
| T-002 메타 보강 | **완료** (PR #3) |
| T-009 화자 수와 분기 | **완료.** 검수 후 Claude 가 보정했다. 아래를 읽는다 |
| T-003 축약 20종 용례 | 대기. **다음 작업** |
...
## 화자 수는 이제 손으로 적지 않는다 (T-009 이후 규칙 변경)

`speakerCount` 와 `quarter` 는 대본에서 기계로 파생된다. 직접 고치지 않는다.

```bash
python3 eng2p/scripts/derive_speakers.py --check   # 집계만 본다
python3 eng2p/scripts/derive_speakers.py --write   # 카탈로그와 대본을 맞춘다
```

대본을 새로 넣거나 고쳤으면 `--write` 를 돌리고 그 결과를 커밋한다.
`check_media.py` 가 카탈로그, 대본 머리말, 본문 계수 셋을 대조한다.
하나라도 어긋나면 실패다.

**왜 바뀌었나.** T-009 는 게이트를 통과했지만 통과 방식이 문제였다.
화자 수 5건이 내려갔고 분기가 8/7/14/23 으로 몰렸다.
Q1 은 소리 트랙이 40퍼센트라 자료가 제일 많이 필요한 분기인데 제일 적어졌다.
규격 위반은 아니었다. 경고를 지우는 최단 경로였을 뿐이다.

합격 기준을 "경고 0" 으로만 준 지시서에도 책임이 있다.
그래서 숫자를 사람 판단에서 빼냈다. 이제 조정할 여지가 없다.

**앞으로 지시서를 읽을 때.** 합격 기준은 최소선이지 목표가 아니다.
가장 짧은 경로로 통과하지 말고 무엇을 위한 조건인지 보고 판단한다.
판단이 갈리면 혼자 정하지 말고 PR 본문의 "판단이 필요했던 것" 에 적는다.

---

## T-009 화자 수 초과 17건 정리 (완료. 검수 후 보정)

담당: GPT
상태: **완료.** 조수 작업 위에 Claude 가 보정했다.

조수가 한 것
- 경고 17건 해소. 화자 수 5건 하향, 분기 17건 이동

검수에서 나온 것
- 대본을 직접 파싱해 세니 정규화 후 7건이 어긋났다. lle1-33 은 선언 6 실제 3
- 분기 분포가 13/13/13/13 에서 8/7/14/23 으로 무너졌다

Claude 가 한 것
- `derive_speakers.py` 신설. 화자 수 7건 정정, 분기 21건 재배치
- 결과 12/12/12/16
- `check_media.py` 가 선언값 대신 대본 계수를 쓰도록 강화
