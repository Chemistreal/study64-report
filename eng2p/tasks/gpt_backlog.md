...
경고 52개는 전부 같은 것이다. 대본이 없다.

---

## T-001 대본 52개 로컬화 (최우선)
...
각 항목의 `page` 링크에 있는 VOA 원문 대본을 가져와
`transcript` 필드에 영어 문자열 배열로 넣는다.

```json
"transcript": [
  "Anna: Hello! I am Anna.",
  "Man: Hi, Anna. Nice to meet you.",
  "Anna: Nice to meet you, too."
]
```

한 줄에 한 발화. 화자 표시를 그대로 살린다.
음성에서 들리는 것과 글자가 어긋나면 안 된다.

### 왜

이게 이 프로젝트에서 지금 제일 값이 크다. 이유가 셋이다.

1. **검사 가능해진다.** 나는 음성을 들을 수 없고 대본은 검사할 수 있다.
   대본 없는 음성은 아무도 검사하지 않은 채로 학습자 귀에 들어간다.
   기준서 1.3이 지목한 구조적 위험이 음성에서 터지는 지점이다.
2. **대본 동기화가 열린다.** english.html 클립 탭에 대본을 줄 단위로
   음성에 맞추는 기능이 이미 있다. 데이터만 오면 바로 붙는다.
3. **T-004 축약 채집의 입력이 된다.** 대본이 있어야 축약이
   어디 나오는지 찾을 수 있다.
...
- 52개 전부 `transcript` 가 있다
- 배열이 비어 있지 않다
- 각 줄이 비어 있지 않다
- **한글이 하나도 없다.** 대본은 영어로만 적는다
- em-dash 와 U+FFFD 가 없다
- catalog.js 와 catalog.json 의 내용이 같다

### 하지 말 것

- 대본을 지어내거나 다듬지 않는다. 원문 그대로 옮긴다
- 들리지 않는 부분을 추측으로 채우지 않는다. 빼고 지시서에 적어 보낸다
- 한국어 번역을 넣지 않는다. 번역은 다른 필드에서도 안 받는다
- english.html 을 건드리지 않는다
