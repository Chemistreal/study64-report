...
규칙은 두 갈래로 나눈다.
    ...
    trim       앞뒤 공백만 지운다 (`.trim()`)

⚠ **이 두 줄을 나란히 세우면 안 된다**(2026-08-11 에 고침). 예전에는
  «trim — KMChC·study64» 라고 적어 두었는데, 재어 보니 그 둘에는 **맞춰 보는
  키가 아예 없다.** 거기 있는 `trim` 은 입력칸에서 앞뒤 공백을 떼는 것으로,
  어느 앱이나 하는 일이고 **같은 학생인지 견주는 자리가 아니다.**
  «규칙이 갈렸다» 와 «규칙이 없다» 는 다른 이야기다 — 앞엣것은 맞추면 되고,
  뒤엣것은 필요한지부터 묻는 일이다.
